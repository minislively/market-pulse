---
name: mp
description: market-pulse CLI를 실행해 터미널 기반 시장 카드, 사용자 해석 피드백, 리뷰를 제공합니다. /mp now, /mp think, /mp review 또는 오늘 시황/시장 펄스/시장 생각 피드백 요청에 사용합니다.
---

# market-pulse Claude Command

`market-pulse`의 로컬 `mp` CLI를 호출하는 얇은 Claude Code slash command입니다.
...
mkdir -p ~/.local/bin
ln -sf ~/Library/Python/3.11/bin/mp ~/.local/bin/mp
```
...
- 선호 구조: mood, drivers, tensions, counter-view, next observation question.
